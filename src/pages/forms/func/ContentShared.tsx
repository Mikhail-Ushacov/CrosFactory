...
export const ProductSelector = ({ 
  ...
  <div className="space-y-3">
    <div className="flex items-center justify-between">
      <label className="text-xs font-bold text-slate-400 uppercase flex items-center gap-2"><Package size={14}/> Прив'язані товари</label>
      ...
      {selectedProducts.map((p) => (
        <div key={p.id} className="flex items-center gap-2 bg-slate-50 border border-slate-100 pl-3 pr-1 py-1 rounded-full group">
          ...
          <div className="flex items-center">
            <button type="button" onClick={() => window.open(`/product/${p.id}`, '_blank')} className="p-1 text-slate-300 hover:text-indigo-600"><ExternalLink size={12} /></button>
            <button type="button" onClick={() => onRemove(p.id)} className="p-1 text-slate-300 hover:text-red-500"><X size={12} /></button>
          </div>
        </div>
      ))}
    </div>
