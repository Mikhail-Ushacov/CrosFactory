...
class NewsService {
  ...
  async getById(id) {
    ...
    if (isNaN(newsId)) {
      const error = new Error("Некоректний ID новини");
    ...
    return {
      ...item,
      images: (item.images || []).map(img => img.image?.url).filter(Boolean),
      contentBlocks: (item.contentBlocks || []).map(block => ({
        ...block,
        images: (block.images || []).map(img => img.image?.url).filter(Boolean),
  ...
  async update(id, body, files) {
    ...
    return await prisma.$transaction(async (tx) => {
      // 1. Видаляємо старі зв'язки (самі картинки в Image залишаються)
      await tx.newsImage.deleteMany({ where: { newsId } });
      await tx.newsBlock.deleteMany({ where: { newsId } });

      // 2. Оновлюємо та створюємо нові блоки з використанням connectOrCreate
